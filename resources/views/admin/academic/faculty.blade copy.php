...
<script>
    ...
    function createFacultyHtml(faculty) {
        return `
            <div class="col-md-6 card faculty-card" data-id="${faculty.id}" data-element="faculty-${faculty.id}">
                ...
                <div class="card-body">
                    ...
                    <div class="text-center mt-3">
                        <button class="add-button" onclick="addBatch(${faculty.id})">
    ...
    function createBatchHtml(batch) {
        return `
            <div class="batch-item nested-item" data-id="${batch.id}" data-element="batch-${batch.id}">
                ...
                <div class="text-center mt-3">
                    <button class="add-button" onclick="addClass(${batch.id})">
    ...
    function createClassHtml(cls) {
        return `
            <div class="class-item nested-item" data-id="${cls.id}" data-element="class-${cls.id}">
                ...
                <div class="text-center mt-3">
                    <button class="add-button" onclick="addSection(${cls.id})">
    ...
    function deleteFaculty(facultyId,type) {
        if (confirm('Are you sure you want to delete this faculty and all its data?')) {
            facultyData = facultyData.filter(f => f.id != facultyId);
            renderFacultyData();
            console.log(`Deleted faculty with ID: ${facultyId}`);
        }
    ...
    function deleteBatch(batchId,type) {
        if (confirm('Are you sure you want to delete this batch and all its data?')) {
            facultyData.forEach(faculty => {
                faculty.batches = faculty.batches.filter(b => b.id != batchId);
            });
            renderFacultyData();
    ...
    function deleteClass(classId,type) {
        if (confirm('Are you sure you want to delete this class and all its data?')) {
            facultyData.forEach(faculty => {
                faculty.batches.forEach(batch => {
                    batch.classes = batch.classes.filter(c => c.id != classId);
                });
            });
            renderFacultyData();
    ...
    function addElementInServer1(newElement,type) {
        console.log(newElement)
        $.ajax({
            url: `/add-element`,  // URL for the controller action
            method: 'POST',
            ...
            success: function(response) {
                if (response && response.data && response.type) {
                    const newElementId = response.data.id; // Get the new ID from the response
                    const elementType = response.type; // Get the type from the response
                    oldElementId = newElement.id;
                        console.log(oldElementId)
                        console.log(newElementId)

                    // Find the element based on its type and set its data-id to the new ID
                    if (type === 'faculty') {
                        const facultyCard = $(`[data-element="faculty-${oldElementId}"]`);
                        facultyCard
                            .attr('data-id', newElementId)
                            .attr('data-element', `faculty-${newElementId}`);
                        // Update the delete button's event handler
                        facultyCard.find('.delete-btn').attr('onclick', `deleteCard(${newElementId}, 'faculty')`);
                        facultyCard.find('.editable-title').attr('data-id', newElementId);
                    } else if (type === 'batch') {
                        const batchCard = $(`[data-element="batch-${oldElementId}"]`);
                ...
                // toastr.success(`${type} title updated successfully!`);
            },
            error: function(xhr, status, error) {
                // toastr.error(`Error updating ${type} title: ${error}`);
